...
public final class DatabaseHelper extends SQLiteOpenHelper {
	...
	public void copyAttachedDatabase(Context context, String databaseName) {
		...
		try {
			...
			ActiveLog.e("Failed to open file", e);
		}
	}
	...
	private void executePragmas(SQLiteDatabase db) {
		if (SQLiteUtils.FOREIGN_KEYS_SUPPORTED) {
			...
			ActiveLog.i("Foreign Keys supported. Enabling foreign key features.");
		}
	}
	...
	private boolean executeMigrations(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		try {
				...
				for (String file : files) {
					try {
						...
						if (version > oldVersion && version <= newVersion) {
							...
							ActiveLog.i(file + " executed succesfully.");
						}
					}
					catch (NumberFormatException e) {
						ActiveLog.w("Skipping invalidly named file: " + file, e);
					}
				}
				db.setTransactionSuccessful();
			}
			...
			ActiveLog.e("Failed to execute migrations.", e);
		}
	...
	private void executeSqlScript(SQLiteDatabase db, String file) {
		...
		try {
			...
			ActiveLog.e("Failed to execute " + file, e);

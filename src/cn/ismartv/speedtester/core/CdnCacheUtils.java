...
public class CdnCacheUtils {
    ...
    public static int getAreaCodeByCdnNick(String cdnNick) {
        if (cdnNick.indexOf(NORTH_CHINA) != -1) {
            return NORTH_CHINA_CODE;
        } else if (cdnNick.indexOf(EAST_CHINA) != -1)
            return EAST_CHINA_CODE;
        else if (cdnNick.indexOf(SOUTH_CHINA) != -1)
            return SOUTH_CHINA_CODE;
        else if (cdnNick.indexOf(CENTRAL_CHINA) != -1)
            return CENTRAL_CHINA_CODE;
        else if (cdnNick.indexOf(WN_CHINA) != -1)
            return WN_CHINA_CODE;
        else if (cdnNick.indexOf(WS_CHINA) != -1)
            return WS_CHINA_CODE;
        else if (cdnNick.indexOf(EN_CHINA) != -1)
            return EN_CHINA_CODE;
        return OTHERS_CODE;
    }

    public static int getIspByCdnNick(String cdnNick) {
        if (cdnNick.indexOf(CHINA_NET) != -1)
            return CHINA_NET_CODE;
        else if (cdnNick.indexOf(CHINA_UNICOM) != -1)
            return CHINA_UNICOM_CODE;
        else if (cdnNick.indexOf(CHINA_MOBILE) != -1)
            return CHINA_MOBILE_CODE;
        else if (cdnNick.indexOf(CHINA_GREATE_WALL) != -1)
